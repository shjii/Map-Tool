...
void	KDevice::SetMode(bool bFullScreen)
{
...
void KDevice::ResizeDevice(UINT w, UINT h)
{
...
HRESULT KDevice::CreateDXResource(UINT w, UINT h)
{
	return S_OK;
}
...
bool KDevice::PreRender()
{
	if (m_pImmediateContext.Get())
	{
		...
		m_pImmediateContext->RSSetState(KDxState::g_pRSBackCullSolid);//KDxState::g_pRS);
		m_pImmediateContext->PSSetSamplers(0, 1, &KDxState::g_pSSWrapLinear);
		m_pImmediateContext->OMSetDepthStencilState(KDxState::g_pDSSDepthEnable, 0);
	}
	return true;
}
bool KDevice::Render()
