...
namespace DX
{
	ID3D11RasterizerState* KDxState::g_pRSWireFrame = 0;
	ID3D11RasterizerState* KDxState::g_pRSSolid = 0;
	ID3D11RasterizerState* KDxState::g_pRSBackCullSolid = 0;
	ID3D11RasterizerState* KDxState::g_pRSNoneCullSolid = 0;
	ID3D11RasterizerState* KDxState::g_pRSFrontCullSolid = 0;
	ID3D11RasterizerState* KDxState::g_pRSSlopeScaledDepthBias;
	ID3D11RasterizerState* KDxState::g_pRS[6] = { 0, };

	ID3D11SamplerState*	 KDxState::g_pTexSS = 0;
	ID3D11SamplerState*  KDxState::g_pSSWrapLinear = NULL;
	ID3D11SamplerState*  KDxState::g_pSSWrapPoint = NULL;
	ID3D11SamplerState*  KDxState::g_pSSMirrorLinear = NULL;
	ID3D11SamplerState*  KDxState::g_pSSMirrorPoint = NULL;
	ID3D11SamplerState*  KDxState::g_pSSClampLinear = NULL;
	ID3D11SamplerState*  KDxState::g_pSSClampPoint = NULL;
	ID3D11SamplerState*  KDxState::g_pSSShadowMap = NULL;
	ID3D11SamplerState*  KDxState::g_pSS[8] = { 0, };

	ID3D11DepthStencilState*	KDxState::g_pDSSDepthEnable = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSSDepthDisable = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSSDepthEnableNoWrite = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSSDepthDisableNoWrite = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSSDepthStencilAdd = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSSDepthAlways = 0;
	ID3D11DepthStencilState*	KDxState::g_pDSS[6] = { 0, };

	ID3D11BlendState*	  KDxState::g_pAlphaBlend = 0;
	ID3D11BlendState*	  KDxState::g_pNoAlphaBlend = 0;
	ID3D11BlendState*	  KDxState::g_pBSColorOne = 0;
	ID3D11BlendState*	  KDxState::g_pBSOneOne = 0;
	ID3D11BlendState*	  KDxState::g_pBSOneZero = 0;
	ID3D11BlendState*	  KDxState::g_pBSAlphaOne = 0;
	ID3D11BlendState*	  KDxState::g_pBSMaxOneOne = 0;
	ID3D11BlendState*	  KDxState::g_pBS[7] = { 0, };

	Microsoft::WRL::ComPtr<ID3D11BlendState>		KDxState::g_pBSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11RasterizerState>	KDxState::g_pRSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11SamplerState>      KDxState::g_pSSEdit = 0;
	Microsoft::WRL::ComPtr<ID3D11DepthStencilState> KDxState::g_pDSSEdit = 0;
	D3D11_BLEND_DESC			KDxState::g_BlendDesc = { 0, };
	D3D11_RASTERIZER_DESC		KDxState::g_RasterizerDesc;
	D3D11_SAMPLER_DESC			KDxState::g_SamplerDesc;
	D3D11_DEPTH_STENCIL_DESC	KDxState::g_DepthStencilDesc = { 0, };

	FLOAT KDxState::g_fBlendFactor[4] = { 0, };
	UINT  KDxState::g_iMask = 0xff;
	UINT KDxState::g_iSlot = 0;
	UINT KDxState::g_iArray = 1;
	UINT KDxState::g_iRef = 1;

	HRESULT KDxState::SetBlendState(ID3D11Device* pd3dDevice,
	...
	HRESULT KDxState::SetRasterizerState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT KDxState::SetSamplerState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT KDxState::SetDipthStencilState(ID3D11Device* pd3dDevice,
		ID3D11DeviceContext* pContext,
	...
	HRESULT KDxState::SetState(ID3D11Device*	pd3dDevice)
	{
		...
		pd3dDevice->CreateBlendState(&blendDesc, &KDxState::g_pBSColorOne);

		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;// D3D11_BLEND_OP_REV_SUBTRACT;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &KDxState::g_pBSOneOne);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;// DEST_COLOR;
		pd3dDevice->CreateBlendState(&blendDesc, &KDxState::g_pBSOneZero);

		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &KDxState::g_pBSAlphaOne);

		blendDesc.AlphaToCoverageEnable = TRUE;
		blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_MAX;
		blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
		blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
		pd3dDevice->CreateBlendState(&blendDesc, &KDxState::g_pBSMaxOneOne);
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSBackCullSolid))) return hr;
		g_RasterizerDesc = rsDesc;

		rsDesc.FillMode = D3D11_FILL_WIREFRAME;
		rsDesc.CullMode = D3D11_CULL_NONE;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSWireFrame)))
			return hr;
		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_BACK;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSSolid)))
			return hr;

		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_NONE;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSNoneCullSolid)))
			return hr;


		rsDesc.FillMode = D3D11_FILL_SOLID;
		rsDesc.CullMode = D3D11_CULL_FRONT;
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSFrontCullSolid)))
			return hr;
		...
		if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &KDxState::g_pRSSlopeScaledDepthBias)))
			return hr;
