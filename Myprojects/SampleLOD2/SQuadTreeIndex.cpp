#include "SQuadTreeIndex.h"
SStaticData::SStaticData()
{
}
SStaticData::~SStaticData()
{
...
bool SQuadTreeIndex::Build(SMap* pMap, DWORD dwWidth, DWORD dwHeight)
{
	m_dwWidth = dwWidth;
	m_dwHeight = dwHeight;
	m_pMap = pMap;
...
bool SQuadTreeIndex::SubDivide(SNode* pNode)
{
...
void SQuadTreeIndex::UpdateBoundingBox(SNode* pNode)
{
...
void	SQuadTreeIndex::ComputeBoungigBox(SNode* pNode)
{
	Vector2 vHeight = GetHeightFromNode(pNode->m_CornerIndex[0], pNode->m_CornerIndex[1],
		pNode->m_CornerIndex[2], pNode->m_CornerIndex[3]);

	pNode->S_BOX.vMax = m_pMap->m_VertexList[pNode->m_CornerIndex[1]].p;
	pNode->S_BOX.vMin = m_pMap->m_VertexList[pNode->m_CornerIndex[2]].p;
	pNode->S_BOX.vMax.y = vHeight.x;
	pNode->S_BOX.vMin.y = vHeight.y;
...
SNode* SQuadTreeIndex::CreateNode(SNode* pParenSNode, DWORD dwTL, DWORD dwTR, DWORD dwBL, DWORD dwBR)
{
	SNode* pNode = 0;
...
Vector2	SQuadTreeIndex::GetHeightFromNode(DWORD dwTL, DWORD dwTR, DWORD dwBL, DWORD dwBR)
{
	assert(m_pMap);
...
int SQuadTreeIndex::UpdateIndexList(SNode* pNode, DWORD dwCurentIndex, DWORD dwNumLevel)
{
...
// 2, 0 ( 좌, 상 )
// 3, 1 ( 우, 하 )
int  SQuadTreeIndex::SetLodIndexBuffer(SNode* pNode,
	DWORD& dwCurentIndex,
...
bool SQuadTreeIndex::Frame()
{
	m_DrawPatchNodeList.clear();
	// PreFrame함수 호출 및 오브젝트 컬링
	if (!SQuadTree::Frame()) return false;
...
int SQuadTreeIndex::UpdateStaticIndexList(SNode* pNode, DWORD dwCurentIndex, DWORD dwLod)
{
	if (!m_IndexList) return false;
	int iNumFaces = 0;
...
DWORD SQuadTreeIndex::GetIndexCounter(DWORD dwData, DWORD dwNumCell)
{
...
bool SQuadTreeIndex::ComputeStaticLodIndex(int iMaxCells)
{
	m_pdwLodIndexArray.reserve(m_iPatchLodCount + 1);

	// 레벨 0
	SStaticData*  pStaticData = NULL;
	SAFE_NEW(pStaticData, SStaticData);
...
DWORD SQuadTreeIndex::GetIndex(DWORD dwIndex, DWORD dwTL, DWORD dwTR, DWORD dwBL, DWORD dwBR, DWORD dwLod)
{
	DWORD dwReturn = 0;
	DWORD dwStartRow = dwTL / m_dwWidth;
	DWORD dwStartCol = dwTL % m_dwWidth;
...
HRESULT SQuadTreeIndex::CreateIndexBuffer(SStaticData*  pData)
{
	HRESULT hr = S_OK;
	...
	for (int iData = 0; iData < pData->IndexList.size(); iData++)
	{
		...
		// 정점 인덱스 버퍼 생성
		pData->IndexBufferList[iData].Attach(TBASIS_CORE_LIB::CreateIndexBuffer(m_pd3dDevice,
...
HRESULT SQuadTreeIndex::CreateVertexBuffer(SNode* pNode, DWORD dwCount, PNCT_VERTEX* pvHeighSMap)
{
	HRESULT hr = S_OK;
	if (dwCount <= 0) return S_OK;
	void** pData = nullptr;
	if (dwCount > 0) pData = (void**)pvHeighSMap;

	pNode->m_pVertexBuffer.Attach(TBASIS_CORE_LIB::CreateVertexBuffer(m_pd3dDevice,
		pData,
		dwCount,
		sizeof(PNCT_VERTEX)));
...
bool SQuadTreeIndex::CreateVertexList(SNode* pNode)
{
	DWORD dwTL = pNode->m_CornerIndex[0];
	DWORD dwTR = pNode->m_CornerIndex[1];
	DWORD dwBL = pNode->m_CornerIndex[2];
	...
	PNCT_VERTEX* pvHeighSMap = 0;
	SAFE_NEW_ARRAY(pvHeighSMap, PNCT_VERTEX, dwSize);
	...
	for (DWORD dwRow = dwStartRow; dwRow <= dwEndRow; dwRow++)
	{
		for (DWORD dwCol = dwStartCol; dwCol <= dwEndCol; dwCol++)
		{
			pvHeighSMap[iVertexIndex++] = m_pMap->m_VertexList[dwRow*m_dwWidth + dwCol];
		}
	}
	if (FAILED(CreateVertexBuffer(pNode, iVertexIndex, pvHeighSMap)))
	{
		return false;
	}
	SAFE_DELETE_ARRAY(pvHeighSMap);
	return true;
}

bool SQuadTreeIndex::Render(ID3D11DeviceContext*    pContext)
{
	...
	for (int iNode = 0; iNode < m_DrawPatchNodeList.size(); iNode++)
	{
		...
		//if (m_bUsedCrossTypeIndex)
		{
			...
			pContext->UpdateSubresource(m_pMap->m_pConstantBuffer.Get(), 0, NULL, &m_pMap->m_cbData, 0, 0);
...
SQuadTreeIndex::SQuadTreeIndex(void)
{
